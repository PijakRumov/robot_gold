...
public:
    LidarFiltrResults apply_filter(std::vector<float> points, float angle_start, float angle_end) {
        constexpr float angle_range = M_PI / 4; //pi/12
...
private:
    ...
    void imu_callback(const sensor_msgs::msg::Imu::SharedPtr msg) {
        ...
        if (state_ == CALIBRATION) {
            ...
            if (std::fabs(delta_yaw) >= M_PI / 3) {
                integrator_.reset();
                state_ = GOING_STRAIGHT_AFTER_TURN;
                go_straight();
                RCLCPP_INFO(this->get_logger(), "Finished turn, now GOING_STRAIGHT_AFTER_TURN");
            } else {
    ...
    void lidar_callback(const sensor_msgs::msg::LaserScan::SharedPtr msg) {
        ...
        // Turn logic (uses possibly clamped values)
        if (need_ && results.right < 0.3f && results.front < 0.3f) {
            start_turning(-1);
            return;
        ...
        if (results.left > 0 && results.right > 0) {
        ...
        float x = pid.step(center_offset, 0.2);
    ...
    void go_straight() {
        ...
        go_straight_timer_ = this->create_wall_timer(
            ...
            [this]() {
                auto now = this->now();
                if ((now - go_straight_start_time_).seconds() >= 1.5) {
                    go_straight_timer_->cancel();
                    if (state_ == GOING_STRAIGHT_AFTER_TURN) {
                        state_ = CORRIDOR_FOLLOWING;
    ...
    void rotate_in_place(int direction) {
        std_msgs::msg::UInt8MultiArray msg;
        uint8_t base = 130;
        msg.data = {static_cast<uint8_t>(base - 20 * direction), static_cast<uint8_t>(base + 20 * direction)};
        motor_pub_->publish(msg);
    }
